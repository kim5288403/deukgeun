...
public class PostServiceImpl implements PostService{
   ...
   * @param html   HTML 이스케이프된 게시글 내용
   */
  @CachePut(value = "post", key = "#userId", cacheManager = "projectCacheManager", unless = "#result == null")
  ...
  public Map<Object, Object> saveImage(HttpServletRequest request, HttpServletResponse response) throws Exception {
    ...
    // 파일 확장자 추출
    String extension = getExtensionFromContentType(filePart.getContentType());
    String name = UUID.randomUUID().toString() + extension;
    String linkName = postUrl + name;

    // MIME 타입 검증
    validMimeType(filePart.getContentType(), uploads, name);
  ...
  public void deletePostImage(String src) {
    ...
    String filename = url[url.length - 1];
    deleteFileToDirectory(postFilePath + filename);
  }
